...
| `sim_bloodline_join.gd` | 血筋キャラの加入イベント(design.md 4.2節、2026-09-24): 雇用候補の血筋の割合(旧家3%・王家1%)、効果「探索者が加入する」(名前・血筋・職業・肖像・初期スキル、雇用上限を超えても加わる、掲示板、肖像が探索者の画像でなければ自動、取り込み時の検査)、デフォルトシナリオの連鎖(旧家の屋敷→イザベラ→貴族街の門、離宮の隠れ家→リュシアン→国境の関所)、セーブ往復 |
| `sim_gate_retry_event.gd` | 会話付きの戦闘ゲートで、初回に負けて撤退した後、日次の再挑戦(`Exploration._retry_gates`)で実際に戦う時にも「前置き → 戦闘画面 → 結末」が再生され、勝てば突破の会話が出ること、会話の間は突破しないこと、勝ち目の無い相手には毎日会話を出さないこと(2026-09-24、「撤退後2回目のバトルイベントが再生されない」の再発防止) |
