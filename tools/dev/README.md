...
| `sim_daily_log.gd` | ログウィンドウ(マップ右端に重ねる。毎日の動き・行動ログ・掲示板をタブで切り替える。2026-09-24)と、毎日の動き(`DailyLog`)の確認: 状態の判定(探索中/回復中/力を付けている/周回中、未割当は出さない)、同じ日の記録し直し、90日の保持、まとめ表示の区間(日が連続しなければ分かれる)、マップ右上のボタンでの開閉・タブ・タブごとの絞り込みの記憶・✕/戻るキーで閉じる・掲示板のスレッドを開く(ポップアップからなら閉じる)・自動更新、探索との連携(会話が閉じて退避が反映された後に、その日の動きが「力を付けている」に記録し直される)、ロード/新規プレイで空から |
...
| `sim_difficulty.gd` | 難易度モード(Easy/Normal/Hard。`difficulty.gd`、design.md 4.9節)の確認: 計算式、パーティの初期値・切替・セーブ往復(メモリ上と、隔離したAPPDATAでのSQLite)・旧セーブ相当、戦闘で勝てる敵の上限がEasy>Normal>Hardで倍率どおりか(予測と実際の戦闘)、スキルゲートの必要Lv、月次収入・周回収入・攻略報酬・予測の倍率(初回の完全攻略ボーナスは固定)、画面(アイコンの左上の頭文字バッジの文字・色・位置、休養中でも出る、ツールチップ、パーティ一覧のカード、詳細の切替ボタンと説明)。メイン画面を組み立てる作りで、実際の戦闘は戦闘力の経験値で戦力が変わるので、上限を測った後に実戦を行う順にしてある |
| `sim_area_follow.gd` | マップの表示が見せたい場所へ切り替わること(`main.gd`の`_focus_map`、design.md 5.1節。2026-09-22、「マップクリアで次のエリアに移動しても、マップが切り替わらない」「イベントの起きたエリアに切り替えたい」の再発防止)の確認: (a)まだ誰も来ていないエリアへ自動で配置転換されると、見ているエリアによらず切り替わる(タブ/タッチUIはプルダウンも) (b)既に来たエリア・手動の割り当て・未割当・同じエリア内の移動では切り替えない (c)イベントの会話が始まると、ゲート/条件(フロア発見・セクション到達)のイベントの場所のエリアへ切り替わり、下のフロアは会話ウィンドウに隠れない位置へスクロールする(日数・フラグだけの条件と案内会話は切り替えない。`ScenarioEvents.event_location`) (d)雇用パーティが初めてエリアに入ると切り替わり、野良の旅人では切り替えない。実際の日次と同じく`TimeSystem.day_advanced`を発火して(探索→画面の順)確かめ、スクロールは数フレーム後に確かめる。タッチUIは末尾に`-- --touch-ui`を付ける。**ヘッドレスでは、エリア切替直後のスクロール範囲の丸め(実ウィンドウで起きた不具合)は再現しない**ので、見た目は`--script`の実ウィンドウ起動+`get_viewport().get_texture()`のスクリーンショットで確かめた |
| `sim_gate_labels.gd` | マップの突破待ちフロアの箱の、解除条件の表示(`main.gd`の`_gate_icon`/`_gate_short_text`/`_refresh_map`、design.md 5.3節)の確認: 鍵開け🔒・知覚🔍・知恵🧩・破壊💥・戦闘⚔️・血筋👑・所持品🎒の文言、突破済み・未発見・ゲート無しの従来表示、長い条件(最長級のアイテム名)で箱が横に広がらないこと(「…」で省略)、フロア詳細のゲート説明のアイコン。メイン画面を組み立て、文字を入れ替えた後のレイアウトが済む数フレーム後に箱の幅を測る作り |
| `sim_battle_screen.gd` | 戦闘画面(`battle_screen.gd`、`main.gd`の`_build_battle_screen_ui`、design.md 5.4節。2026-09-22)の確認: `Combat.resolve_party_encounter`のtraceが数値計算に副作用を与えないこと(外してもresult/HP/経験値は不変、predict_party_resultと一致)、会話付きの戦闘ゲートで設定ONなら開き設定OFF/勝ち目が無い場合は開かないこと、開いている間`TimeSystem.dialogue_hold`が立つこと、タップ(正しいクリックイベントのみ)/早送り/閉じるの操作、back-keyでは閉じないこと、退避の説明会話を横取りしないこと(戦闘画面が塞いでいる間は待ち、閉じた後に`call_deferred`で改めて開く)。境界条件(満タンなら勝てるが今のHPでは勝てない状況)は、初期パーティの固有スキル(乱数)とmax_hpを揃えたうえで、`predict_party_result`を使ってLvを実際に探索する作り(手計算は敵の戦闘力が減っていく非線形な式のため当てにならない) |
