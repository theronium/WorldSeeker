...
# 探索者の肖像画像。design.md 4.2節の血筋(NameGenerator.NAMES_BY_BLOODLINE)と同じ抽選パターンで、
...
# 黒い騎士)は、通常の探索者肖像として使うには不自然なため、抽選プールからは除外した(画像ファイル
...
# 同じフォルダには、イベント会話用の町人画像(npc_01〜32)と敵画像(enemy_01〜32)も入っている
# (2026-09-20追加)。これらは血筋のプールには入れない(探索者の肖像としては使わない)。
# 選び方はevent_portraits.gd。
