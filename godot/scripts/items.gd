...
## 転職(design.md 4.8節)などの消費アイテム用。所持していなければ何もせずfalseを返す。
func consume(npc_id: int, item_id: String) -> bool:
	var npc := Npcs.get_npc(npc_id)
	if npc.is_empty() or not npc["inventory"].has(item_id):
		return false
	npc["inventory"].erase(item_id)
	return true
